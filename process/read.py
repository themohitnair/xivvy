...
class Parser:
    def __init__(self):
        ...
        self.latex_patterns = {
            ...
            'references': re.compile(r"(Figure|Fig\.|Eq\.|Equation|Section|Table)\s+\d+(\.\d+)?", re.IGNORECASE),
            'references2': re.compile(r"(Eq\.|Equation|Fig\.|Figure|Table|Section)\s*\(\d+\)", re.IGNORECASE),
    ...
    def normalize_category(self, category: str) -> str:
        ...
        for valid_cat in self.valid_categories:
            if category.startswith(valid_cat + ".") or category.startswith(valid_cat + "-"):
    ...
    async def _process_line(self, line: str) -> ExtractedPaper | None:
        ...
        try:
            ...
            try:
                paper_id = obj.get("id", "").strip() if isinstance(obj.get("id"), str) else ""
                title = (obj.get("title") or "").strip() if isinstance(obj.get("title"), str) else ""
                abstract = (obj.get("abstract") or "").strip() if isinstance(obj.get("abstract"), str) else ""
                categories_raw = obj.get("categories", "").strip() if isinstance(obj.get("categories"), str) else ""
                update_date = obj.get("update_date", "").strip() if isinstance(obj.get("update_date"), str) else ""
            except AttributeError as e:
                self.logger.error(f"Type error in paper fields: {e} - Paper ID: {obj.get('id', 'unknown')}")
                return None
            ...
            if not (title or abstract):
                self.logger.warning(f"Skipping paper due to missing title AND abstract: {paper_id}")
                return None
                    ...
                    for cat in categories_raw.split():
                        if cat:
                            try:
                                ...
                                self.logger.warning(f"Error normalizing category '{cat}': {cat_error}")
                    # Convert to list and sort
                    categories = sorted(category_set)
            except Exception as cat_error:
                self.logger.error(f"Error processing categories for {paper_id}: {cat_error}")
            ...
            try:
                ...
                if not combined_text.strip():
                    self.logger.warning(f"Paper {paper_id} has empty content after sanitization")
                ...
                combined_text = combined_text_raw[:1500] if combined_text_raw else title or abstract

            # Validate update date format
            if update_date and not self._is_valid_date_format(update_date):
                self.logger.warning(f"Invalid date format for paper {paper_id}: {update_date}")
            ...
            try:
                ...
                self.logger.error(f"Error creating ExtractedPaper for {paper_id}: {model_error}")
                return None
    ...
    async def parse_yield_batches(self) -> AsyncIterator[List[ExtractedPaper]]:
        ...
        batch = []
        line_buffer = []
        buffer_size = min(1000, self.batch_size * 2)  # Process multiple lines in parallel
        lines_processed = 0
        papers_extracted = 0
        ...
        try:
            ...
            try:
                                ...
                                for result in results:
                                    ...
                                    if result:  # Valid paper
                                        ...
                                        if len(batch) >= self.batch_size:
                                            self.logger.info(f"Yielding batch of {len(batch)} papers")
                                            yield batch
                                            batch = []
                ...
                if line_buffer:
                    try:
                        ...
                        self.logger.error(f"Error processing final batch: {final_batch_error}")
            ...
            if batch:
                self.logger.info(f"Yielding emergency batch of {len(batch)} papers after error")
                yield batch
